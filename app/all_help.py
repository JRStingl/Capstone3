...
def get_pickle(imdbid, imdb_movies):
    ...
    for i in range(len(lst)):
        if ind < lst[i]:
            files = (lst[i])+1
            break
    dbfile = open(f'../distance/weights_full_set{int(files)}.pkl', 'rb')
...
def clean_up(movie_min, message, test, year, duration, toy_story, imdb_movies, poster_images):
    ...
    looking_for=10
    print(genre)
    m_test = imdb_movies[(imdb_movies.original_title==test)&(imdb_movies.year==year)].title_id.item()
...
#     top_ten = imdb_movies.iloc[recommends].original_title[0:10].values
    ...
    new_ids=[]
    for id, title in zip(ids,top_ten):
        if id[0]=='s':
